    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `𝗵𝗮𝗹𝗹𝗼 𝗯𝗼𝘀 @${num.split('@')[0]} 👋\n 𝘀𝗲𝗹𝗮𝗺𝗮𝘁 𝗱𝗮𝘁𝗮𝗻𝗴 𝗱𝗶 𝗴𝗿𝘂𝗽 *_${metadata.subject}_*\n 𝗶𝘀𝗶 𝗱𝘂𝗹𝘂 🤗\n\n*Nama:*\n*Umur:*\n*Askot:*\n\n*Deskripsi Grup:*\n\n${metadata.desc}` })
                } else if (anu.action == 'remove') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `𝗯𝘆𝗲 𝗯𝗲𝗯𝗮𝗻 @${num.split('@')[0]} 👋\n *Kalo balik lagi nitip gorengan ya*\n\n*_Sisa ${participants.length} Member_*` })
                } else if (anu.action == 'promote') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Cie Admin ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Kasihan Di Un admin ${metadata.subject}` })
              }
            }
        } catch (err) {
            console.log(err)
